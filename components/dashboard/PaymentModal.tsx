...
export default function PaymentModal({ isOpen, onClose, onSuccess, type, tierId, tierNom, totalDu, invoices }: PaymentModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!selectedInvoceId || !montant) return showError('Veuillez remplir tous les champs.')

    setLoading(true)
    try {
      const endpoint = type === 'VENTE' ? '/api/reglements/ventes' : '/api/reglements/achats'
      const payload = type === 'VENTE' 
        ? { venteId: selectedInvoceId, montant: Number(montant), modePaiement, observation }
        : { achatId: selectedInvoceId, montant: Number(montant), modePaiement, observation }
