...
async function runFinalAudit() {
    console.log("=== AUDIT TECHNIQUE DE PERSISTENCE GESTICOM (FINAL) ===");
    const results = [];
    ...
    try {
        ...
        const magasin = await prisma.magasin.findFirst() || await prisma.magasin.create({ data: { nom: "Audit Mag", code: "AM-" + Date.now() } });
        const entite = await prisma.entite.findFirst() || await prisma.entite.create({ data: { nom: "Audit Entite", code: "AE-" + Date.now() } });
        const user = await prisma.utilisateur.findFirst() || await prisma.utilisateur.create({ data: { nom: "Audit User", login: "audit-" + Date.now(), password: "pwd", role: "SUPER_ADMIN" } });
        ...
        // 5. Trésorerie (Caisse / Dépenses)
        await check("Caisse", async () => {
            // Utiliser le bon champ pour transaction
            const c = await prisma.caisse.findFirst() || await prisma.caisse.create({ data: { nom: "Audit Caisse", code: "AC-" + Date.now(), magasinId: magasin.id } });
            const t = await prisma.transaction.create({ data: { 
                libelle: "Audit Caisse", 
                montant: 100, 
                type: "ENTREE", 
                caisseId: c.id, 
                utilisateurId: user.id,
        ...
        await check("Dépenses", async () => {
            const d = await prisma.depense.create({ data: { 
                libelle: "Audit Depense", 
                montant: 100, 
                categorie: "DIVERS", 
                utilisateurId: user.id,
                modePaiement: "ESPECES",
        ...
        await check("Comptabilité", async () => {
            const e = await prisma.ecritureComptable.create({ data: { 
                date: new Date(), 
                libelle: "Audit Compta", 
                journalCode: "OD", 
                utilisateurId: user.id,
                numero: "AUDIT-E-" + Date.now()
            } });
        ...
        console.error("ERREUR GLOBALE D'AUDIT :", globalError.message);
    }
    ...
    if (failed.length === 0 && results.length > 0) {
        ...
        console.log(`❌ ${failed.length} ERREURS DÉTECTÉES SUR ${results.length} TESTS.`);
    }
