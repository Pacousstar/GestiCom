...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const mouvements = await prisma.mouvement.findMany({
            ...
            include: {
                produit: { select: { designation: true, prixAchat: true, prixVente: true } }
            }
        })

        const stockEntree = mouvements.filter(m => m.type === 'ENTREE').reduce((acc, m) => acc + (m.quantite * (m.produit.prixAchat || 0)), 0)
