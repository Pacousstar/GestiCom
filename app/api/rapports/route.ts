...
export async function GET(request: NextRequest) {
  ...
  if (hasDates && Array.isArray(topData)) {
    ...
    topProduitsAll = sorted.map(([produitId, q]) => ({
      produitId,
      code: prodMap.get(produitId)?.code ?? '',
      designation: prodMap.get(produitId)?.designation ?? '',
      quantiteVendue: q,
    }))
  } else {
    ...
    const prodMap = new Map(prods.map((p) => [p.id, p]))
    topProduitsAll = sorted.map((g) => ({
      produitId: g.produitId,
      code: prodMap.get(g.produitId)?.code ?? '',
      designation: prodMap.get(g.produitId)?.designation ?? '',
  ...
  if (hasDates && deb && fin) {
    ...
    const montantAchatsActuels = Number(achatsActuelsResult._sum.montantTotal || 0)
    const ventesActuellesCount = ventesActuelles._count.id || 0

    const caPrecedent = Number(ventesPrecedentes._sum.montantTotal || 0)
    const montantAchatsPrecedents = Number(achatsPrecedentsResult._sum.montantTotal || 0)
